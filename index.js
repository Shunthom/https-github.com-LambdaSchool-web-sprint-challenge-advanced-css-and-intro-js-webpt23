...
const name = "getArtist";
const bio = "getBio";
function getArtist(artists, i) {
  getArtist = artists[i];
  return getArtist;
}
console.log("Amedeo Modigliani");

function getBio(artists, i) {
  getBio = artists[2];
  return getBio;
}
console.log(getBio);
...
let newName = artists.indexOf("Vincent van Dough");
{
 ...
 */
function removeArtist(array, removalArtist) {
  const artistIndex = array.findIndex(
    (indexValue) => indexValue === removalArtist
    //console.log(indexValue);
  );
  array.splice(artistIndex, 0);
  return array;
}
